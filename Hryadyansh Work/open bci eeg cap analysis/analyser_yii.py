...
high_cutoff = 15 
EPOCHS_TIMING = (0, 1.2)
eeg_stream_index_list = []
for i  in range(len(streams)):
    print(streams[i]['info']['name'][0])
    if streams[i]['info']['name'][0] == 'openbci_eeg':
        eeg_stream_index_list.append(i)
    elif streams[i]['info']['name'][0] == 'MyMarkerStream':
        marker_stream_index = i

eeg_stream_index = None
for i in eeg_stream_index_list:
    if streams[i]['time_stamps'][0] <= streams[marker_stream_index]['time_stamps'][0]:
       eeg_stream_index = i
       break
    
if eeg_stream_index is None:
    raise ValueError('No EEG stream found that starts after the marker stream.')
...
epochs = mne.Epochs(raw, events_filtered, tmin=EPOCHS_TIMING[0], tmax=EPOCHS_TIMING[1], baseline=(0, 0), preload=True, event_repeated='drop')
print(epochs)
