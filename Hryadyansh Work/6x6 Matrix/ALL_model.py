...
class EEGNet_ELU(torch.nn.Module):
    def __init__(self, n_output=2, Samples=1000, Chans=3, kernLength=500, F1=8, D=2, F2=16):
        super(EEGNet_ELU, self).__init__()
        self.Chans = Chans  # Added Chans as an instance variable

        self.firstConv = nn.Sequential(
            nn.Conv2d(1, F1, kernel_size=(1, kernLength), stride=(1, 1), padding=(0, kernLength // 2), bias=False),
            nn.BatchNorm2d(F1, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        )
        self.depthwiseConv = nn.Sequential(
            nn.Conv2d(F1, F1 * D, kernel_size=(Chans, 1), stride=(1, 1), groups=F1, bias=False),
        ...
        self.separableConv = nn.Sequential(
            nn.Conv2d(F1 * D, F2, kernel_size=(1, 16), stride=(1, 1), padding=(0, 8), bias=False),  # Changed kernel_size to (1, 16)
        ...
        self.classify = nn.Sequential(
            nn.Flatten(),
            nn.Linear(F2 * ((Samples // 4) // 8), n_output, bias=True)  # Assuming Samples is a global variable
        )
...
# Example usage for EEG NET ELU:
# kernLength = 500 # Should be equal to sfreq*0.5
# F1 = 8
# D = 2
# F2 = F1 * D
# Chans = 3  # Define the number of channels
# Samples = 1000  # Define the number of samples, samples = sfreq * total_time
# n_output = 2
